...
## 2026-08-04 — Gambles: three risk cards prototype push-your-luck reform

Trigger: the designer's ask — cards with a probability of success driven by popularity, improvable with money (but limited), failure costing popularity or a sector's income — which finally houses the "push your luck" note from the idea list. Prototyped with **three cards, deliberately not a full set**: the popularity system is a day old and unplaytested, so the gamble mechanic ships at minimum viable size (one popularity-risk, one sector-income-risk, one negative-offset long shot) to be judged on the stories it produces before expanding.

The rules, and why each is shaped that way:

1. **Odds = popularity + per-card offset.** No hidden formula — your approval rating *is* the chance the reform passes (Referendum ±0; Industry Transition Pact +10, industry likes a deal; Fossil Phase-Out Act −20, deeply divisive). This makes popularity matter *continuously* where gates only bite at thresholds, and turns the popularity bar into an odds display for free.
2. **Campaign boosts: +5% per 2M$, max 3, total capped at 90%.** Money improves odds but never buys certainty — certainty is what deterministic cards are for.
3. **One attempt per run, consumed win or lose.** Retryable gambles invite odds-grinding, which is solitaire, not politics. Failure applies a printed backlash in the domain of the reform (botched restructuring → Industry income −1/turn perm; failed referendum → −10% popularity); success EV is priced modestly above the deterministic equivalent as compensation for variance (Pact: Process Electrification's outcome at 5M$ vs 9M$, at ~70-80% odds).
4. **The anti-suicide invariant extends to dice.** A new `risk_floor` blocker refuses any attempt whose failure could land below the collapse threshold — "only crises can bring a government down" survives randomness, and the failure cost doubles as a soft popularity gate with zero new rules.
5. **Dice are as traceable as everything else.** Rolls come from the run's seeded RNG (determinism preserved) and are logged ("rolled 71 vs 68% — the reform fails"); tests verify exact branches by cloning the RNG state to predict the roll.

UI: the game's first **two-step purchase** — risk cards open a gold-bordered confirm dialog (odds recolored by band, both outcomes, boost button with n/3 counter, live total cost) instead of buying on click; resolution flashes a pass/fail banner via a `risk_resolved` signal. The clean bot only gambles at ≥70% odds; greedy gambles on anything.

Harness result (40 seeds) carried two surprises: **the clean bot's 2/40 stall seeds vanished** — the 2030 Referendum is a popularity engine, so the ≥60% gated combo pieces now unlock — and **always-mortgage dropped back below always-pay** (34 vs 38 wins), restoring the intended archetype ordering. Do-nothing still loses 2042; absorb still dies (39/40 popularity collapses). Standing watch item unchanged: greedy matches clean. Playtest questions for the trio: does the 90% cap frustrate or thrill, is one-attempt-per-run legible without the tooltip, and does the Phase-Out Act's −20 offset read as "build popularity first" or as a trap?
