...
func test_pair_combo_fires() -> void:
	...
	approx(rs.money, m0 - 90.0 + 40.0, 1e-9, "combo reward +40 at x1.0")
...
func test_combo_once_per_turn_recharges_next_turn() -> void:
	var rs := _fresh()
	rs.play_card(&"TRA2")
	rs.play_card(&"IND1")
	...
	var m0 := rs.money
	rs.play_card(&"IND1")
	eq(rs.combos_total, 2, "same combo fires again next turn")
	eq(rs.combo_chain, 2, "chain builds across turns")
	approx(rs.money, m0 - 90.0 + 40.0 * 1.1, 1e-6, "chain 1 => x1.1 reward")
...
func test_combo_effects_applied() -> void:
	...
	rs.play_card(&"RSP2")  # water -> Water Cycle: sink_now 0.3 (+0.1 from RSP2)
	eq(rs.combos_total, 1, "Water Cycle fired")
	approx(rs.absorption, a0 + 0.1 + 0.3, 1e-9, "combo sink effect applied on top of the card")
...
func test_chain_decay_on_comboless_turn() -> void:
	var rs := _fresh()
	rs.play_card(&"TRA2")
	rs.play_card(&"IND1")
	eq(rs.combo_chain, 1, "chain 1 after the combo")
	rs.resolve_year()  # comboless turns decay the chain
	rs.pending_crises = []
	rs.resolve_year()
	eq(rs.combo_chain, 0, "chain decays by 1 on a comboless turn (min 0)")
	rs.resolve_year()
...
func test_chain_cap_multiplier() -> void:
	...
	rs.play_card(&"TRA2")
	rs.play_card(&"IND1")
	approx(rs.money, m0 - 140.0 - 90.0 + 40.0 * 2.0, 1e-6,
...
func test_combos_unlock_cards() -> void:
	...
	for i in 4:
		...
		rs.resolve_year()
		rs.pending_crises = []
	eq(rs.combos_total, 4, "four combos over four turns")
